package com.dcm.easypoi.excel.export.template;

import com.dcm.easypoi.cache.ExcelCache;
import com.dcm.easypoi.entity.ImageEntity;
import com.dcm.easypoi.excel.ExcelExportUtil;
import com.dcm.easypoi.excel.annotation.ExcelTarget;
import com.dcm.easypoi.excel.entity.TemplateExportParams;
import com.dcm.easypoi.excel.entity.TemplateSumEntity;
...
import com.dcm.easypoi.excel.export.base.BaseExportService;
import com.dcm.easypoi.excel.export.styler.IExcelExportStyler;
import com.dcm.easypoi.excel.html.helper.MergedRegionHelper;
import com.dcm.easypoi.exception.excel.ExcelExportException;
import com.dcm.easypoi.exception.excel.enums.ExcelExportEnum;
import com.dcm.easypoi.util.*;
...
public final class ExcelExportOfTemplateUtil extends BaseExportService {
    ...
    private ExcelForEachParams getIndexColumn(List<ExcelForEachParams> columns) {
        for (int i = 0; i < columns.size(); i++) {
            if (columns.get(i) != null && PoiElUtil.INDEX.equals(columns.get(i).getConstValue())) {
        ...
        try {
            ...
            for (Integer sheetNum : sheetNumList) {
                ...
                for (int i = mapList.size() - 1; i >= 0; i--) {
                    ...
                    if (mapList.get(i).containsKey(ExcelExportUtil.SHEET_NAME)) {
                        wb.setSheetName(sheetIndex, mapList.get(i).get(ExcelExportUtil.SHEET_NAME).toString());
                    }
            ...
            for (int i = row.getFirstCellNum(); i < row.getLastCellNum(); i++) {
                ...
                if (row.getCell(i) != null && (cell.getCellType() == CellType.STRING
                    ...
                    if (text.contains(PoiElUtil.IF_DELETE)) {
                        if (Boolean.valueOf(
                                PoiElUtil.eval(text.substring(text.indexOf(PoiElUtil.START_STR) + 2, text.indexOf(PoiElUtil.END_STR))
                                        .trim(), map).toString())) {
    ...
    private void setValueForCellByMap(Cell cell, Map<String, Object> map) throws Exception {
        ...
        if (oldString != null && oldString.indexOf(PoiElUtil.START_STR) != -1
            ...
            boolean isNumber = isHasSymbol(oldString, PoiElUtil.NUMBER_SYMBOL);
    ...
    private Object getValByHandler(String funStr, Map<String, Object> map, Cell cell) throws Exception {
        // step 2. 判断是否含有解析函数
        if (isHasSymbol(funStr, PoiElUtil.NUMBER_SYMBOL)) {
            funStr = funStr.replaceFirst(PoiElUtil.NUMBER_SYMBOL, "");
        }
        boolean isStyleBySelf = false;
        if (isHasSymbol(funStr, PoiElUtil.STYLE_SELF)) {
            isStyleBySelf = true;
            funStr = funStr.replaceFirst(PoiElUtil.STYLE_SELF, "");
        }
        boolean isDict = false;
        String dict = null;
        if (isHasSymbol(funStr, PoiElUtil.DICT_HANDLER)) {
            isDict = true;
            dict = funStr.substring(funStr.indexOf(PoiElUtil.DICT_HANDLER) + 5).split(";")[0];
            funStr = funStr.replaceFirst(PoiElUtil.DICT_HANDLER, "");
            funStr = funStr.replaceFirst(dict + ";", "");
        }
        boolean isI18n = false;
        if (isHasSymbol(funStr, PoiElUtil.I18N_HANDLER)) {
            isI18n = true;
            funStr = funStr.replaceFirst(PoiElUtil.I18N_HANDLER, "");
        }
        boolean isDern = false;
        String dern = null;
        if (isHasSymbol(funStr, PoiElUtil.DESENSITIZATION_RULE)) {
            isDern = true;
            dern = funStr.substring(funStr.indexOf(PoiElUtil.DESENSITIZATION_RULE) + 5).split(";")[0];
            funStr = funStr.replaceFirst(PoiElUtil.DESENSITIZATION_RULE, "");
            funStr = funStr.replaceFirst(dern + ";", "");
        }
        if (isHasSymbol(funStr, PoiElUtil.MERGE)) {
            String mergeStr = PoiPublicUtil.getElStr(funStr, PoiElUtil.MERGE);
            funStr = funStr.replace(mergeStr, "");
            mergeStr = mergeStr.replaceFirst(PoiElUtil.MERGE, "");
            try {
        ...
        Object obj = funStr.indexOf(PoiElUtil.START_STR) == -1 ? PoiElUtil.eval(funStr, map) : PoiPublicUtil.getRealValue(funStr, map);
        if (isDict) {
        ...
        for (int k = 0; k < rowspan; k++) {
            ...
            for (int i = 0; i < colspan && i < columns.size(); i++) {
                ...
                if (StringUtils.isEmpty(params.getName())) {
                    ...
                    isNumber = isHasSymbol(tempStr, PoiElUtil.NUMBER_SYMBOL);
    ...
    private ExcelForEachParams getExcelTemplateParams(String name, Cell cell,
        ...
        //是否是当前索引
        if (PoiElUtil.INDEX.equals(name)) {
            params.setName(null);
            params.setConstValue(PoiElUtil.INDEX);
        }
        //获取合并单元格的数据
